<template>
    <div id="search">
      ...
      <div class="closeSearch" v-if="clLists.length">
          <h4>最近搜索</h4>
          <div class="closeList">
            <span v-for="(item,index) in clLists" :key="index"><a href="javascript:void(0);" :data-id="item.cat_id">{{item.content}}</a></span>
          </div>  
      </div>
      <div class="hotSearch closeSearch">
          <h4>热门搜索</h4>
          <div class="closeList">
            <span v-for="(item,index) in hotLists" :key="index"><a href="javascript:void(0);" :data-id="item.cat_id">{{item.content}}</a></span>
          </div>  
      </div>
